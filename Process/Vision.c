...
/*int VISION_PrcessFrame(VisionRange range,VisionRange range1,VisionRange range2, BlobCoord *blob, char *gameplay,char *color)// detect 3 color, return 2 blob coordinate (yellow, blueand red)
{
	...
	int chanels_hsv = imgHSV->nChannels;
	 ...
	 for(a=0;a<imgHSV->height;a++)//cut window in Y [0,240]
        for(b=0;b<imgHSV->width;b++)//cut window in X [30-300] for (b=30;b<300; b++)
        {
...
        
		...
		{
			cnt1++;// file 0 data read in. if fulfil if, then cnt1=1 and it is 1st color
...
            }	   
		...
		{
			cnt2++;// file 1 data read in. if fulfil else if, then cnt2=1 and it is 2nd color
...
            }	   
		...
		{
			cnt3++;// file 2 data read in. if fulfil else if, then cnt3=1 and it is 3rd color
...
            }
	...
	if(cnt2 != 0)//OBS Yellow, file 1n (UI bar 1)
	{
		...
		//printf("Yellow: (%d , %d) (%d , %d)\n", rect.x,rect.y,rect.x + rect.width,rect.y + rect.height);
		        blob1->Xmin = rect.x / 2;				
				...
				blob1->Ymin = rect.y;
				blob1->Ymax = rect.y + rect.height;   // yellow refer to blob
		}
		else
		{
				blob1->Xmin = 255;				
	...
	if(cnt1!=0 && cnt3!=0) //both red and blue are detected, file 0, red and file 2, blue
	{
	    
	     if(cnt1!=0)
	     {
		...
		// blob red and blue by comparing	
		for(o=0;o<3;o++)
		{
			for(p=0;p<3;p++)
			{
